...
/* creates a string representing a single test */
char* format_test(test *t, ub2 len) {
...
/* creates an array of String representing the set of all generated tests */
Output format_all(state *s) {
	Output output;
	char** tests = (char**) malloc(sizeof(char*) * s->ntests);
	for (int i = 0; i < s->ntests; ++i) {
		tests[i] = format_test(s->t[i], s->ndim);
	}
	output.length = s->ntests;
	output.tests = tests;
	return output;
}
...
void build_tuples( state *s, ub2 d, ub2 f)
{
  ...
  for (;;) {
    ...
    if (!insert_tuple(s, &ctx, tuple)) {
      ...
      // RB
      /*
      printf("tuple of size %ld, for %d, %d: ", n, d, f);
      for(int ql = 0; ql < n; ql++)
          printf("%d%d ", tuple[ql].d, tuple[ql].f);
      printf("\n");
       */
      // ~ RB
...
ub4 count_tuples( state *s, test *t, int d, int f)
{
  ...
  ub1      n = s->n[d][f];
  tu_iter  ctx;
  feature *zis = start_tuple(&ctx, &s->tu[d][f], n, &s->tc[d][f]);
  while (zis) {
    count += test_tuple(t->f, zis, n);
    zis = next_tuple(&ctx);
  }
  return count;
}
...
ub4 generate_test( state *s, test *t, feature *tuple, ub1 n)
{
    ...
    printf("\n generating test for: ");
    for (int i=0; i<n; i ++) {
        printf("%d%d", tuple[i].d, tuple[i].f);
    }
     */
    // ~RB
    ...
    if (!covered) {
      ...
      for (d=0; d<s->ndim; ++d) {
          ...
          for (f=0; f<s->dim[d]; ++f) {
              ...
              while (zis) {
                  if (subset_tuple(extra, tuple_n, zis, n)) {
                      zis = delete_tuple(&ctx);
                  } else {
                      zis = next_tuple(&ctx);
                  }
              }
      ...
      for (d=0; d<s->ndim; ++d) {
          ...
          /* remove all the tuples covered by it */
          while (zis) {
              if (test_tuple(best_test->f, zis, n)) {
                  zis = delete_tuple(&ctx);
              } else {
                  zis = next_tuple(&ctx);
              }
...
Output generator(int argc, char *argv[])
{
    ...
    if (parse(argc, argv, &s)) {               /* read the user's instructions */
        ...
        if (confirm(&s)){       /* doublecheck that all tuples really are covered */
            result = format_all(&s);                                  /* report the results */
        }
        else
